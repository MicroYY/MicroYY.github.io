...
DDA（Digital Differential Analyzer）是一种利用直线的微分方程来生成直线的算法。其核心在于把“绘制一天直线”的任务拆解为“每走一步，计算一下坐标变化”的过程。
...
**核心思想：**
1. 确定主位移方向：比较 $|\Delta x|$ 和 $|\Delta y|$，谁更长就以谁为步进基准（$\text{one step}$）。
...
2.  **整数化魔法**：
    ...
    **最终的整数算法逻辑**：
    ...
    这正是你在后续代码中C++看到的 `err += 2 * std::abs(y1 - y0)` 和 `if (err > (x1 - x0))` 的来源。

#### 2. 通用化实现

上述推导仅适用于 $x$ 变化大于 $y$ 的情况（缓坡）。对于陡峭的直线（$|\Delta y| > |\Delta x|$），直接步进 $x$ 会导致线条断裂（因为 $y$ 可能会一次跳变多个像素）。
