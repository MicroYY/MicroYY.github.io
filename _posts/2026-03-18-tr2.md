...
**为什么光栅化代码里的三角形（`math::Triangle3f`）使用的是的 3D 顶点？**
尽管我们在屏幕像素空间画图（看起来是个纯 2D 的填充问题），但现代光栅化不仅要“决定点亮屏幕上的哪个像素”，更重要的责任是计算这个像素后续着色所需的**深度与材质信息**。
在将顶点送进 `DrawTriangle` 之前，$x$ 和 $y$ 坐标已经被世界转换管线（MVP、Viewport）映射成了诸如 $(800, 600)$ 的屏幕 2D 物理网格点；而伴随它们流转下来的 $z$ 坐标则被坚决保留，代表了该像素点映射回 3D 空间里的“原原本本的远近”。这个生死攸关的深度信息 $z$，正是随后利用 Z-Buffer 算法解决不同片段间“近不挡挡远不挡”遮挡关系必须用到的原素材。
